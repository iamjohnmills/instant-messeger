(async () => {
  ...
  server.events.on('onSocketMessage', ({ client, event }) => {
    if(event.connect){
      createSocketUser({ client: client, reconnect: event.reconnect, user_name: event.user_name, room_name: event.room_name })
    } else if(event.chat) {
  ...
  const createSocketUser = ({ client, reconnect, user_name, room_name }) => {
    try {
      ...
      client.send(JSON.stringify({ success: true, connect: true, reconnect: reconnect, data: { user_name: user_name, room_name: room_name } }));
      const buddies = server.getSocketClientsByRoom({ room_name: room_name }).forEach(buddy => {
        if(previous_name){
          ...
          buddy.send(JSON.stringify({ success: true, buddy_join: true, reconnect: reconnect, data: { user_name: user_name } }));
        }
      });
